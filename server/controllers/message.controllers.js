...
//       acc[user._id.toString()] = { id: user._id, email: user.email || "N/A", name: user.name || "Unknown" };
//       return acc;
//     }, {});
...
//         acc[lawyer._id.toString()] = { id: lawyer._id, email: lawyer.email || "N/A", name: lawyer.name || "Unknown" };
//         return acc;
//       }, {});
//     }
...
const getMessage = async (req, res) => {
  try {
    ...
    // Fetch users and lawyers based on sender IDs
    const users = await User.find({ _id: { $in: uniqueSenderIds } });
    const lawyers = await Lawyer.find({ _id: { $in: uniqueSenderIds } });
    ...
    const lawyerMap = lawyers.reduce((acc, lawyer) => {
      acc[lawyer._id.toString()] = lawyer;
      return acc;
    }, {});

    // Prepare receiver-focused response
    const messageUserData = await Promise.all(
      messages.map(async (message) => {
        ...
        if (userMap[senderId]) {
          // Sender is a user, fetch corresponding lawyer
          receiver = await Lawyer.findOne({ _id: message.receiver_id });
        } else if (lawyerMap[senderId]) {
          // Sender is a lawyer, fetch corresponding user
          receiver = await User.findOne({ _id: message.receiver_id });
        }

...
const createConversation = async (req, res) => {
    try {
        console.log("req",req);
        const { senderId, receiverId, senderType, receiverType } = req.body;  // Extract values from request body
...
const getconversation = async (req, res) => {
    try {
        ...
        const conversationUserData = await Promise.all(conversations.map(async (conversation) => {
            ...
            return {
                talker: user ? { email: user.email, name: user.name } : null,  // Return the user details
